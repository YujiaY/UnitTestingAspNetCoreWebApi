...
namespace EmployeeManagement.Test
{
    public class PromotionsControllerTests
    {
        ...
        public async Task CreatePromotion_RequestPromotionForEligibleEmployee_MustPromoteEmployee()
        {
            ...
            PromotionResultDto promotionResultDto = Assert.IsType<PromotionResultDto>(
                okObjectResult.Value);
            Assert.Equal(expectedEmployeeId, promotionResultDto.EmployeeId);
            Assert.Equal(currentJobLevel + 1, promotionResultDto.JobLevel);
            
            // Fluent Assertions
            OkObjectResult okObjectResult2 = result.Should()
                .BeOfType<OkObjectResult>().Subject;
            PromotionResultDto promotionResultDto2 = okObjectResult2.Value
                .Should()
                .BeOfType<PromotionResultDto>().Subject;
            promotionResultDto2.EmployeeId.Should().Be(expectedEmployeeId);
            promotionResultDto2.JobLevel.Should().Be(++currentJobLevel);
        }
    }
}
